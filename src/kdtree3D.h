...
struct KdTree
{
    ...
    void insertHelper(Node** node, uint depth, std::vector<float> point, int id)
	{
		if(*node==NULL)
			*node = new Node(point,id);
			...
			uint cd = depth % 3;//judge even or old, if even choose the x, else choose the y
	...
	std::vector<int> search(VPoint target_point, float distanceTol)
	{
		std::vector<int> ids;
		std::vector<float> target = {target_point.x, target_point.y, target_point.z};
